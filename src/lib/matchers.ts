...
import { R9SDefaultError } from "../errors/r9sdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | R9SDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | R9SDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new R9SDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new R9SDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
