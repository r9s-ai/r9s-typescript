...
export function geminiEmbedContent(
  ...
  Result<
    models.GeminiEmbedContentResponse,
...
> {
  const input: models.EmbedContentRequest = {
    model: model,
  ...
  const parsed = safeParse(
    input,
    (value) => z.parse(models.EmbedContentRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.GeminiEmbedContentRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1beta/models/{model}:embedContent")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GeminiEmbedContentResponse$inboundSchema),
