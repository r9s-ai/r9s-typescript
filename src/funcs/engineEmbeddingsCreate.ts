 ...
 */
export function engineEmbeddingsCreate(
  client: R9SCore,
  model: string,
...
> {
  const input: models.CreateEngineEmbeddingRequest = {
    model: model,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      z.parse(models.CreateEngineEmbeddingRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.EngineEmbeddingRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.EmbeddingResponse$inboundSchema),
