...
export function geminiBatchGenerateContent(
  ...
  Result<
    models.GeminiBatchGenerateContentResponse,
...
> {
  const input: models.BatchGenerateContentRequest = {
    model: model,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      z.parse(models.BatchGenerateContentRequest$outboundSchema, value),
  ...
  const body = encodeJSON("body", payload.GeminiBatchGenerateContentRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/v1beta/models/{model}:batchGenerateContent")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GeminiBatchGenerateContentResponse$inboundSchema),
