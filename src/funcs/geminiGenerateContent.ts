...
export function geminiGenerateContent(
  ...
  Result<
    models.GeminiGenerateContentResponse,
...
> {
  const input: models.GenerateContentRequest = {
    model: model,
  ...
  const parsed = safeParse(
    input,
    (value) => z.parse(models.GenerateContentRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v1beta/models/{model}:generateContent")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GeminiGenerateContentResponse$inboundSchema),
